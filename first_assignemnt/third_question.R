...
# We have four numerical variables (Price, Weight, SSD and Ppi) so there are few models we can test: 
...
## Assumptions check (we do not have to check multicolinearity as we are only using one independent variable)
...
# As the pval(3.73e-09)<0.05 we have enough evidence to reject the null hypothesis, the assumption of normality of the residuals is not fulfilled.
...
# In this case the p-val(0.223)>0.05 so we have enough evidence for accepting the null hypothesis, the assumption of normality in the residuals is fulfilled.
...
# With these tests we can be sure that residuals follow a normal distribution (p-val>0.05). 

# In this case we can see that SSD is a better predictor than Ppi for Price. We do not take weight into account as it does not fulfill the assumptions and the results are not reliable.
...
# The assumption of normality of the residuals is not fulfilled as the p-val(0.0014)<0.05, the results of this model are not reliable.
...
## Homoscedasticity:
bptest(ws_model)
# As the p-val(0.0324)<0.05 we have enough evidence for rejecting the null hypothesis, so we can state that the Homoscedasticity assumption is not fulfilled.
...
## Homoscedasticity:
bptest(ps_model)
# As the p-val(0.2116)>0.05 we have enough evidence for accepting the null hypothesis, so we can state that the Homoscedasticity assumption is fulfilled.

## Normality of the residuals
plot(ps_model, 2)
shapiro.test(ps_model$residuals)
# The assumption of normality of the residuals is fulfilled as the p-val(0.1543)>0.05.

## No Multicolinearity
# In order to test correlation we can check the variance inflation factor
vif(ps_model)
# The correlation between Ppi and SSD is 1.33, we can state that the assumption of not multicolinearity between the independent variables (Ppi and SSD) is fulfilled

# With this results we could state that the best model for predicting Price is Ppi+SSD, as it has the higher Adjusted R-squared. We can do a further checking with a partial F-test.
...
for (fact in c("Company", "TypeName", "Ram", "TouchScreen", "Ips", "Cpu_brand", "HDD", "Gpu_brand", "Os")) {
  ...
  psf_model <- lm(as.formula(formula_str), data = data)  # Convert the string to a formula
  print(summary(psf_model))  # Print the summary of the model
}
